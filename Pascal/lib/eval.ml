...
let rec eval_binop op v1 v2 =
  let ( <|> ) f1 f2 arg = try f1 arg with _ -> f2 arg in
  let ( => ) arg f = f arg in
  let error =
    PascalInterp (BinOpTypeError (op, get_type_val v1, get_type_val v2))
  in
  let v1, v2 =
    let evolution v =
      match v with
      ...
      | v -> v
    in
    if compare_types (get_type_val v1) (get_type_val v2) then (v1, v2)
    else
  ...
  | VString v1, VString v2 ->
      ( (fun (op, x, y) ->
          (match op with Add -> String.concat x [ ""; y ] | _ -> raise error)
          => fun v -> VString v)
      <|> fun (op, x, y) ->
        (match op with
        | Greater -> x > y
...
let%test "eval binop and" =
  eval_binop And (VBool true) (VBool false) = VBool false
...
let eval_binop_type op t1 t2 =
  ...
  let types =
    match t2 with (VTInt | VTChar) as t2 -> (t2, t1) | t2 -> (t1, t2)
  in
  match op with
  | Add -> (
      match types with
  ...
  | Div | Mod | RShift | LShift -> (
      match types with VTInt, VTInt -> VTInt | _ -> raise error)
  | And | Or | Xor -> (
      match types with
      | VTInt, VTInt -> VTInt
...
let eval_unop op v =
  ...
  match op with
  | Plus -> ( match v with (VInt _ | VFloat _) as ok -> ok | _ -> raise error)
  | Minus -> (
      match v with
      | VInt v -> VInt (-v)
      | VFloat v -> VFloat (-.v)
      | _ -> raise error)
  | Not -> ( match v with VBool v -> VBool (not v) | _ -> raise error)

let eval_unop_type op t =
  let error = PascalInterp (UnOpTypeError (op, t)) in
  match op with
  | Plus -> (
      match t with VTInt -> VTInt | VTFloat -> VTFloat | _ -> raise error)
  | Minus -> (
      match t with VTInt -> VTInt | VTFloat -> VTFloat | _ -> raise error)
  | Not -> ( match t with VTBool -> VTBool | _ -> raise error)

let eval_std_function n p =
  let error = PascalInterp (NotAStdFunction n) in
  let type_error =
    PascalInterp (StdFunctionTypeError (n, List.map get_type_val p))
  in
  match n with
  | "abs" -> (
      match p with
  ...
  | "trunc" -> (
      ...
      | _ -> raise type_error)
  | "chr" -> (
      match p with [ VInt v ] -> VChar (Char.chr v) | _ -> raise type_error)
  | "ord" -> (
      match p with [ VChar v ] -> VInt (Char.code v) | _ -> raise type_error)
  | _ -> raise error
...
let eval_std_function_type n pt =
  ...
  | "arctan" | "cos" -> (
      ...
      | _ -> raise type_error)
  | "round" | "trunc" -> (
      match pt with [ VTFloat ] -> VTInt | _ -> raise type_error)
  | "chr" -> ( match pt with [ VTInt ] -> VTChar | _ -> raise type_error)
  | "ord" -> ( match pt with [ VTChar ] -> VTChar | _ -> raise type_error)
  | _ -> raise error
...
let%test "get_rec test" =
  get_rec
    (VRecord
       (KeyMap.of_seq (List.to_seq [ ("i", (VTInt, VVariable (VInt 42))) ])))
    "i"
  = VInt 42
...
let get_arr v ind =
  ...
  | VArray (start, size, _, arr) ->
      ...
      if act_ind < 0 || act_ind >= size then
        raise (PascalInterp (ArrayOutOfInd (get_type_val v, ind)))
...
let%test "eval expr partal" =
  eval_expr_const KeyMap.empty (Const (VInt 42)) = VInt 42

let%test "eval expr 1 + 1" =
  eval_expr_const KeyMap.empty (BinOp (Add, Const (VInt 1), Const (VInt 1)))
  = VInt 2
...
let%test "eval expr const variables" =
  eval_expr_const
    (KeyMap.of_seq
       (List.to_seq
          [ ("x", (VTInt, VConst (VInt 32))); ("y", (VTInt, VConst (VInt 10))) ]))
    (BinOp (Add, Variable "x", Variable "y"))
  = VInt 42

let%test "eval expr const round" =
  eval_expr_const KeyMap.empty (Call (Variable "round", [ Const (VFloat 3.) ]))
  = VInt 3
