...
module ImArray = struct
  type 'a t = Created of 'a Array.t | Empty of int * 'a

  let use f = function
    | Created a -> f a
    | Empty (size, value) -> f (Array.make size value)

  let length = function Created a -> Array.length a | Empty (s, _) -> s

  let get t ind =
    match t with Created a -> Array.get a ind | Empty (_, value) -> value
