 ...
 fun def ->
  ...
  let def_to_world w =
    ...
    let rec load_type = function
      ...
      | VTDString i ->
          if i > 0 then VTDString i else raise (PascalInterp TypeError)
      | VTRecord l ->
          let list_to_map =
    ...
    let rec construct = function
      ...
      | VTDString _ -> VString ""
      | VTDRecord w ->
          VRecord (KeyMap.map (fun t -> (t, VVariable (construct t))) w)
      | VTFunction _ -> VVoid
    ...
    | DFunction (n, t, p, (d, c)) ->
        ...
        ( n,
          ( VTFunction (p, t),
            const (VFunction (n, t, p, load_variables fdef, c)) ) )
  in
  let add_to_world w d =
    let name, value = def_to_world w d in
    if KeyMap.mem name w then raise (PascalInterp (DupVarName name))
