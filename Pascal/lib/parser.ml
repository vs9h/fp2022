...
let token s =
  word <|> token s >>= function x when x = s -> return s | _ -> mzero
...
let name =
  let* result = word in
  match use_parser key_word result with None -> return result | _ -> mzero
...
let value =
  ...
  let float =
    ...
    let* float_part =
      exactly '.'
      >> check_next (function Some x -> x != '.' | _ -> true)
      >> many digit
    in
    return (float_of_string (cl2s (int_part @ ('.' :: float_part))))
  in
  let symbol = any >>= function '\\' -> any | '\'' -> mzero | c -> return c in
  let char =
    let ascii =
...
let%test "value char ascii 65 (A)" =
  check_parser value "\'@65\'" (VChar (Char.chr 65))
...
let%test "value string" =
  check_parser value "\'foo \\\'bar\\\'\'" (VString "foo \'bar\'")
...
let%test "a + b - c" =
  check_parser expr "a + b - c"
    (BinOp (Sub, BinOp (Add, Variable "a", Variable "b"), Variable "c"))

let%test "a + - b * c" =
  check_parser expr "a + - b * c"
    (BinOp
       (Add, Variable "a", BinOp (Mul, UnOp (Minus, Variable "b"), Variable "c")))

let%test "(a[1, 2][3] shl b) * c" =
  check_parser expr "(a[1, 2][3] shl b) * c"
    (BinOp
       ( Mul,
         BinOp
           ( LShift,
...
let%test " - func(a . b.c, 3) [ c ]" =
  check_parser expr " - func(a . b.c, 3) [ c ]"
    (UnOp
       ( Minus,
         GetArr
           ( Call
...
let rec statement s =
  ...
  let for_st =
    ...
    let* finish = expr in
    let* body = token "do" >> statement_block in
    if direction then return (For (control_variable, start, finish, body))
...
let%test "assignment test" =
  check_parser statement "a.b := 42 + 5"
    (Assign
       (GetRec (Variable "a", "b"), BinOp (Add, Const (VInt 42), Const (VInt 5))))

let%test "if _ then statement" =
  check_parser statement "if a then b:=10"
    (If (Variable "a", [ Assign (Variable "b", Const (VInt 10)) ], []))

let%test "if _ then begend else begend" =
  check_parser statement "if a then begin b := 15; end else begin h := 8 end"
    (If
       ( Variable "a",
...
let%test "repeat until" =
  check_parser statement "repeat a := a + 1; b := a - 1; until a < 128"
    (Repeat
...
let rec definition s =
  ...
  let compress_DType_list f lst =
    let res =
      List.fold_right
        ...
        lst (Some [])
    in
    match res with Some l -> return l | None -> mzero
  in
  let rec function_arg proc s =
    let helper =
      let fun_param =
        ...
        let fun_param_const =
          token "const" >> helper (fun n tp -> FPConst (n, tp))
        in
      ...
      let* params =
        lexeme
          (between (token "(") (token ")")
             (sep_by fun_param (many1 (token ";")))
          <|> return [])
        => List.concat
      in
      if proc then return (VTFunction (params, VTVoid))
      else
  ...
  and vtype s =
    let string_arg =
      option VTNDString
        (between (token "[") (token "]") expr => fun e -> VTString e)
    in
    let arr_arg =
      let interval =
        let* start = expr in
      ...
      in
      let* intervals =
        between (token "[") (token "]") (sep_by1 interval (token ","))
      in
      let* arr_type = token "of" >> vtype in
      return
        (List.fold_right (fun (s, f) t -> VTArray (s, f, t)) intervals arr_type)
    in
    let record_arg =
      sep_by as_var (many1 (token ";"))
      << many (token ";")
      << token "end" => List.concat
...
let%test "definition 2" =
  check_parser definition "var a, b: integer;"
...
let%test "definition 3" =
  check_parser definition
    ...
    [
      DType
        ("add", VTFunction ([ FPFree ("x", VTInt); FPOut ("y", VTInt) ], VTInt));
    ]

let%test "definition 4" =
  check_parser definition "var p : procedure (const x : string);"
    [ DNDVariable ("p", VTFunction ([ FPConst ("x", VTNDString) ], VTVoid)) ]

let%test "definition 5" =
  check_parser definition "var i : integer = 42;"
    [ DVariable ("i", VTInt, Const (VInt 42)) ]

let%test "definition 6" =
  check_parser definition "var r : record i : integer; f : real; end;"
    [ DNDVariable ("r", VTRecord [ ("i", VTInt); ("f", VTFloat) ]) ]

let pascal_program = program << token "." << many any

let parse s =
  match use_parser pascal_program s with Some (t, _) -> Some t | _ -> None

let%test "Hello world" =
  check_parser pascal_program "begin writeln(\'hello world\'); end."
    ( [],
      [
